...
//refer to https://gist.github.com/rtv/4989304
...
{
  const int myid = *((int *)id); // force the pointer to be a 64bit integer

  const int workloops = myid;
  for (int i = 0; i < workloops; i++)
  {
    printf("[thread %d] working (currStep/totalStep: %d/%d)\n", myid, i, workloops);
...
int main(int argc, char **argv)
{
  ...
  for (int t = 0; t < NUMTHREADS; t++)
    pthread_create(&threads[t], NULL, ThreadEntry, &t);
